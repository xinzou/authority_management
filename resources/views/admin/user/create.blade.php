                                ...
                                <form class="form-horizontal" action="{{URL::route('user.store')}}" method="post">
                                    <fieldset>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-10">
                                                <input class="form-control" id="username" name="username" type="text" value="@if (!empty(Session::get('username'))){{Session::get('username')}}@endif">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-lg-2 control-label" for="truename">姓  名</label>
                                            <div class="col-lg-10">
                                                <input class="form-control" id="truename" name="truename" type="text" value="@if (!empty(Session::get('truename'))){{Session::get('truename')}}@endif">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-lg-2 control-label" for="mobile">移动电话</label>
                                            <div class="col-lg-10">
                                                <input class="form-control" id="mobile" name="mobile" type="text" value="@if (!empty(Session::get('mobile'))){{Session::get('mobile')}}@endif">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-lg-2 control-label" for="landline">email</label>
                                            <div class="col-lg-10">
                                                <input class="form-control" id="email" name="email" type="text" value="@if (!empty(Session::get('email'))){{Session::get('email')}}@endif">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-10">
                                                <select name="role_id" class="form-control" style="width: 150px">
                                                    @foreach ($roleList as $role)
                                                        <option value="{{$role->role_id}}"@if (Session::get('role_id') === $role->role_id) selected="true"@endif>{{$role->role_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
