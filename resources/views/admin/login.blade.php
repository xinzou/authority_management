...
<div class="container">
    <div class="row">
        <div class="col-lg-12">
            ...
            @endforeach
            <form method="post" action="{{URL::action('Admin\LoginController@login')}}@if (Input::has('callback'))?callback={{Input::get('callback')}}@endif" class="bootstrap-admin-login-form">
                <input type="hidden" name="_token" value="{{csrf_token()}}"/>
                <h1>管理系统</h1>
                <div class="form-group">
                    <input class="form-control" type="text" name="username" placeholder="Username" value="@if (!empty(Session::get('username'))){{Session::get('username')}}@endif">
                </div>
                <div class="form-group">
                    <input class="form-control" type="password" name="password" placeholder="Password" value="@if (!empty(Session::get('password'))){{Session::get('password')}}@endif">
                </div>
                <div class="form-group">
                    <label>
